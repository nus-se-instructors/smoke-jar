...
for root, dirs, files in os.walk("jars"):
    for jarname in files:
        if jarname.endswith(".jar"):    #confirm
            ...
            os.chdir("jar_test")
            isOk = False
            jarproc = subprocess.Popen([VIRTUAL_DESKTOP_PATH, JAVA_PATH, "-jar", jarname],stdin=subprocess.PIPE,stdout=subprocess.PIPE,stderr=subprocess.PIPE, shell=False)
            try:
                poll = jarproc.wait(1)
